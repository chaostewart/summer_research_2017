...
cursor.execute("Drop table if exists chao_draft.NHL_season_stats_1998_2016_original;")

mydb.commit()

cursor.execute("""Create table chao_draft.NHL_season_stats_1998_2016_original (PlayerID INT, PlayerName VARCHAR(50), Season VARCHAR(15), GameType VARCHAR(20), Team VARCHAR(35), Position VARCHAR(10), GP INT, G INT, A INT, P INT, PlusMinus INT, PIM  INT, PointPerGame DOUBLE, PPG INT, PPP INT, SHG INT, SHP INT, GWG INT, OTG INT, S INT, ShootingPercentage DOUBLE, TOIPerGame TIME, TOIPerGame_sec INT, ShiftsPerGame DOUBLE, FaceoffWinPercentage DOUBLE);""")

mydb.commit()

# where unimported csv files are stored
inputDir = "/home/cla315/work_yeti/new_wilson_98_16/seasonstats_csvfiles"
# csv files will be moved to hear after being imported to database
moveToDir = "/home/cla315/work_yeti/new_wilson_98_16/seasonstats_oldcsvfiles"
