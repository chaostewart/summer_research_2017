...
'''
cursor.execute("Drop table if exists chao_draft.before_NHL_skaters_stats_1998_2008;")

mydb.commit()

cursor.execute("""Create table chao_draft.before_NHL_skaters_stats_1998_2008
(PlayerId INT, PlayerName VARCHAR(50), BirthDate DATE, BirthYear INT, BirthMonth INT,
BirthDay INT, Birthplace VARCHAR(70), Country VARCHAR(10), Height VARCHAR(10),
Height_inches INT, Weight INT, Position VARCHAR(10), Shoots VARCHAR(10),
DraftYear INT, RoundNumber INT, RoundRank INT, Overall INT, Season VARCHAR(15),
	...
	with open(inputDir + "/" + fileName,'r')  as inputFile:
		...
		for row in csv_data:
			if firstLine:
				firstLine = False
				continue
			cursor.execute("""INSERT INTO chao_draft.before_NHL_skaters_stats_1998_2008
			(PlayerId, PlayerName , BirthDate, BirthYear, BirthMonth, BirthDay, Birthplace , Country, Height,
Height_inches, Weight, Position, Shoots, DraftYear , RoundNumber , RoundRank , Overall , Season ,
Team, GameType, GP, G, A, P, PlusMinus, PIM, PPG, PPP, SHG, SHP,
GWG, OTG, S, ShootingPercentage, FaceoffWinPercentage) VALUES(%s, %s, %s, %s, %s, %s, %s,
 %s, %s, %s, %s, %s, %s, %s,
 %s, %s, %s, %s, %s, %s, %s,
 %s, %s, %s, %s, %s, %s, %s,
 %s, %s, %s, %s, %s, %s, %s)""", row)
