...
for fileName in fileNameList:
	...
	with open(inputDir + "/" + fileName + ".txt",'r') as betting_lines:
		...
		for line in betting_lines:
			...
			rowDict['Height_inches'] = foot * 12 + inch

			weightList = rowDict['Weight'].split(' ')
			rowDict['Weight'] = weightList[0]

			rowDict['RoundNumber'] = rowDict['RoundNumber'][:-5]
	...
	fieldNames = ['PlayerId', 'PlayerName', 'BirthDate', 'BirthYear', 'BirthMonth', 'BirthDay', 'Birthplace', 'Country',
				  'Height', 'Height_inches', 'Weight','Position', 'Shoots', 'DraftYear','RoundNumber', 'RoundRank','Overall','Season',
				  'Team','GameType', 'GP', 'G', 'A', 'P', '+/-', 'PIM', 'PPG', 'PPP', 'SHG', 'SHP', 'GWG', 'OTG', 'S', 'S%', 'FOW%' ]
