...
cursor.execute("Drop table if exists chao_draft.m5p_prediction_1st_cohort;")

mydb.commit()

cursor.execute("""Create table chao_draft.m5p_prediction_1st_cohort
...
for file in os.listdir(inputDir):
    if file.endswith("1st_cohort.csv"):
	...
	with open(inputDir + "/" + fileName,'r')  as inputFile:
		...
		for row in csv_data:
			if firstLine:
				firstLine = False
				continue
			cursor.execute("""INSERT INTO chao_draft.m5p_prediction_1st_cohort
